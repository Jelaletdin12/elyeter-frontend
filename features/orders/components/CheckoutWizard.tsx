  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected ? 'border-primary bg-primary/[0.04] shadow-sm' : 'border-border bg-card',
      )}
    >
      {selected && (
      ...
      {badge && (
        <span className="bg-muted text-muted-foreground mt-3 rounded-full px-2 py-1 text-[10px] font-medium">
          {badge}
        </span>
      )}
    </button>
...
export function CheckoutWizard() {
  ...
  const { data: cart, isLoading: cartLoading } = useQuery(cartOptions(storeId));
  ...
  const hasBlockingIssue = cart?.items.some((item) => item.availableQuantity <= 0) ?? false;
        ...
        <div className="space-y-6">
          ...
          <section className="border-border bg-card rounded-2xl border p-5 shadow-sm sm:p-6">
            ...
            {fulfillmentType === 'DELIVERY' && (
              <div className="mt-6">
                ...
                {!savedAddressId && (
                  <div className="border-border bg-muted/20 mt-4 grid gap-3 rounded-xl border p-4">
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div>
          ...
          <section className="border-border bg-card rounded-2xl border p-5 shadow-sm sm:p-6">
            ...
            <div className="border-border overflow-hidden rounded-xl border">
              <div className="border-border bg-muted/20 flex items-center justify-between gap-4 border-b px-4 py-3">
            ...
            {/* Notice */}
            <div className="bg-primary/[0.05] mt-5 rounded-xl p-4">
          ...
          <div className="border-border bg-card overflow-hidden rounded-2xl border shadow-sm">
            ...
            <div className="space-y-1 p-4">
              {cart.items.map((item) => {
                ...
                return (
                  <div key={item.id} className="hover:bg-muted/30 rounded-xl p-2 transition-colors">
                    <div className="flex gap-3">
                      {/* Product placeholder */}
